...
- [x] SerpApi Mock 클라이언트 (한국 LCC 5개사, 12개 노선 Mock 데이터)
- [x] LccClient 인터페이스 (Mock/Real 전환: serpapi.mock=true/false)
- [x] Duffel + LCC 병렬 검색 병합 (FlightSearchService)
- [x] Kiwi → SerpApi 설정 전환

### 다음 작업
- [ ] 유연한 날짜 검색 엔드포인트
- [ ] Redis 캐싱
- [ ] 프론트엔드 UI 전면 재작성 (frontend-design 스킬 사용)
- [ ] SerpApi 실제 연동 (Mock 전환 후)
